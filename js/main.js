...
const productosDB = [
    ...
    { 
        ...
        tags: ["componentes"], 
        precio: 160000, 
        imagen: "Imagenes/Productos/PlacaMadreGigabyteB760MLGAMicroATX.jfif" 
    },
    { 
        id: 17, 
...
enlacesFiltro.forEach(enlace => {
    enlace.addEventListener('click', (e) => {
        ...
        if (filtro.includes('producto') || filtro === 'todos' || filtro === 'catalogo') {
        ...
        } else if (filtro.includes('placa') || filtro.includes('video') || filtro.includes('placas-video')) {
        ...
        } else if (filtro.includes('periferico')) {
            productosFiltrados = productosDB.filter(p => p.categoria === 'perifericos' || (p.tags && p.tags.includes('perifericos')));
        } else {
...
function renderizarCatalogo(productosAMostrar = productosDB) {
    ...
    if (sortSelect && sortSelect.value) {
        ...
        productosOrdenados.sort((a, b) => {
            if (criterio === 'low-high') {
                return a.precio - b.precio;
            } else if (criterio === 'high-low') {
